    ...
    const fetchUpiSettings = async () => {
      try {
        ...
        if (response.success && response.settings) {
          setUpiId(response.settings.upiId || "")
          setUpiName(response.settings.upiName || "")
          setUpiEnabled(response.settings.upiEnabled || false)
        }
      } catch (error: any) {
                  ...
                  <>
                    <img 
                      ...
                      onError={(e) => {
                        ...
                        fallback.className = 'w-48 h-48 bg-gray-100 flex flex-col items-center justify-center text-gray-500 text-sm rounded-md border border-dashed border-gray-300';
                        fallback.innerHTML = `
                          <div class="text-center">
                            <QrCode class="w-12 h-12 mx-auto mb-2 text-gray-400" />
                            <p>QR Code Preview</p>
                            <p class="text-xs mt-1">UPI ID: ${upiId}</p>
                          </div>
                        `;
